...
public class GuiDemo extends JFrame{
	...
	DrawPanel drawPanel;
	SimpleFileChooser fileChooser;
	TextMenu textMenu;
	JCheckBoxMenuItem gradientOverlayCheckbox = new JCheckBoxMenuItem("Gradient Overlay", true);
	...
	public GuiDemo() {
		...
		/*
		 * The constructor in GuiDemo.java sets up the background and text that 
		 * appear in the picture when the program starts. Personalize the program 
		 * by using your own saying and choice of background. 
		 * I want to be able to tell different people's programs apart! 
		 */
		// Create the DrawPanel that fills most of the window, and customize it.
		drawPanel = new DrawPanel();
		drawPanel.getTextItem().setText("Welcome to my first Image Editor.");
		drawPanel.getTextItem().setFontSize(36);
		drawPanel.getTextItem().setJustify(TextItem.LEFT);
		drawPanel.setBackgroundImage(Util.getImageResource("images/cloud.jpeg"));
		...
		/*
		 * Making a Toolbar part 2:
		 * Find the place in the constructor of the GuiDemo class where the icon toolbar 
		 * is added to the window. Add a background toolbar in the same way, but placing 
		 * it in the BorderLayout.NORTH position instead of the BorderLayout.SOUTH position.
		 */
		...
		/* 
		 * Making a Menu part 2:
		 * Once you have a method to create the menu, find the place in the 
		 * constructor of the GuiDemo class where the menu bar is created. 
		 * Use the method to create a menu, and add that menu to the menu bar. 
		 */
	...
	/*
	 * Making a Toolbar part 1:
	 * Make a JToolBar that duplicates the functionality of the "Background" 
	 * menu, except for the JCheckBox that is at the bottom of the menu. 
	 */
	
	JToolBar makeToolbar(boolean horizontal) {
		JToolBar makeToolbar = new JToolBar( horizontal? JToolBar.HORIZONTAL : JToolBar.VERTICAL);
	...
	AbstractAction newPictureAction = 
	...
	AbstractAction saveImageAction = 
    ...
    public class ChooseBackgroundAction extends AbstractAction {
    	...
    	ChooseBackgroundAction(String text) {
    		...
    		if (!text.equals("Color...")) {
    			 ...
    			 * The "Custom..." and "Color..." actions don't look so good in the toolbar. 
    			 * Since they don't have icons, their text is shown instead. It would be 
    			 * nice to use icons instead; and, actually, it would be nice for them to 
    			 * have icons in the "Background" menu as well. 
    			 * Use the resource "resources/action_icons/fileopen.png" to make an icon 
    			 * for the "Custom..." action. Add this icon to the action where it is 
    			 * created in the constructor of the ChooseBackgroundAction class (at the 
    			 * bottom of GuiDemo.java). 
    			 */
    			if (text.equals("Custom...")) {
    				putValue(Action.SMALL_ICON, 
        					Util.iconFromResource("resources/action_icons/fileopen.png"));
    				putValue(Action.SHORT_DESCRIPTION, 
    						"<html>Select an image file<br>to use as the background.</html>");		
    			}
    			else {
    				putValue(Action.SMALL_ICON, 
    		...
    		/* 
    		 * Making an ImageIcon from Scratch:
    		 * To make the image, create a new BufferedImage with width 32, height 32, and 
    		 * type BufferedImage.TYPE_INT_RGB. Create a Graphics for drawing on the image, 
    		 * and use it to draw the image. For example, fill the entire image with dark 
    		 * gray, then fill a smaller square with light gray (or make some other drawing, 
    		 * such as the red/green/blue bars in my program). 
    		 */
    		else {
    			BufferedImage color = new BufferedImage(32,32,BufferedImage.TYPE_INT_ARGB);
